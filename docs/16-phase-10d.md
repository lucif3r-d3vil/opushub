...
## Part 3 — Final 10D review (security + architecture)

`server/phase10d-review.test.js` (13 proofs) is the executable form of this review. What it establishes:

- **Catalog → normal container.** A Gitea install (volume, env, healthcheck, restart policy, proxy network, published SSH port, reverse-proxy metadata) is read back through `GET /api/services`, `GET /api/services/:group/:name` and `GET /api/v1/containers/:name/spec` like any other container, then goes through `container.stop/start/restart/update/edit/recreate/change_image/remove`, a Diun webhook → `container.update_available`, `POST /api/container-updates/{dry-run,apply}` (Update Now recreates it through the shared ladder, pulling first), an Autoheal webhook → `container.autoheal.restarted`, and `evaluateEligibility` honours `opushub.update=false`. No operation record on that path names catalog code; the catalog runner has no lifecycle code beyond install.
- **Proxy abstraction.** Only `server/proxy/provider.js` contains Traefik label names; planner/spec/policy/engine/UI/manifests carry canonical `expose` (`{domain, port, https, scheme}`) and `integrations.proxy` only. Proxy labels survive `container.edit`/`recreate`/Update Now because they are ordinary labels on the container.
- **Install security.** Static proofs: no `child_process`/`eval`/`vm`, no socket or `requestJson` use, no CLI mentions in catalog code; the runner's engine writes are exactly `{pullImage, createNetwork, createVolume, createContainer, connectNetwork, startContainer, inspectContainer, deleteContainer, removeNetwork}` on the frozen adapter; the manifest schema is a closed key list with no privileged/caps/devices/pid/ipc/security_opt/sysctls/hooks/binds slot. Runtime: `host`/`none`/`container:` networks and undeclared ports/volumes are refused at plan time; binds are absolute-path-only and go through the same `classifySpec` policy as the editor (BLOCKED → 422, DANGEROUS → strong confirmation).
- **Registry trust.** `registries/auth.js` is a re-export of the 10D-C client's `authHeaderFor`; the catalog never reads a secret, never builds `X-Registry-Auth`, and a mismatched `registryId` yields an anonymous pull.
- **Boundary.** Name clash with a stack member → 409 before any write; an existing network (e.g. `proxy`) is joined, never created or removed; a volume the operator points at is attached, never recreated or removed, even on rollback; the wire log shows no write to any pre-existing container; fleet/network/volume snapshots are unchanged around the whole rollback matrix.
- **Operation boundary.** 401 without a session, 403 for a viewer, 409 without a token; the record carries `confirmation.consumed`, `mode: 'strong'`, `verification.verified`, `auditId`, a trail, an `operation.completed`/`operation.failed` Event Bus entry and an activity line.
- **Secret hygiene.** A password variable is absent from dry-run, operation record, trail, `/api/activity`, `/api/v1/events`, `/api/v1/operations`, catalog history/detail, `/plan` preview, the container spec read, every file under `DATA_DIR`, and captured `console.*` output — and present only in the container's `Env`.
- **Rollback matrix** (mock faults: `nonexistent` tag, `fail-net`, `fail-vol`, `fail-create`, `exit-immediately`, `become-unhealthy`, monitor limit): each fault leaves no container, no install-created network, a `failed` history row, no monitor, a failure event and activity line; data volumes are kept and the report says so; a monitor-registration failure is a reported partial success (running container kept, `report.monitor.result = 'skipped'`).

Codebase sweep (duplicates / dead code):

| Finding | Classification |
|---|---|
| Two engine-write HTTP clients: `providers/dockerOperations.js` (Phase 8 control verbs) and `updates/recreateAdapter.js` (10C/10D control adapter, used by all 10D runners) | KEEP_SEPARATE (deliberate: the adapter is the only create/remove path; a mechanical proof depends on it) — NEEDS_FOLLOW_UP to fold the Phase 8 verbs into the adapter in a later phase |
| Two `${var}` substitutions: `stacks/compose.js#substitute` (Compose grammar incl. `:-` defaults) and `catalog/template.js#render` (plain, no defaults, declared vars only) | KEEP_SEPARATE (different grammars by design; catalog intentionally rejects defaults/nesting) |
| Policy: one classifier (`containers/policy.js#classifySpec`) reused by container planner, stack policy and catalog planner | no duplication |
| Registry client: one (`registries/client.js`); `auth.js` is a re-export | no duplication |
| Proxy: read side `providers/dockerLabels.js#parseTraefik`, write side `proxy/provider.js` | KEEP_SEPARATE (read vs write); NEEDS_FOLLOW_UP to move both under `proxy/` |
| Activity/Event writes: every 10D module uses `activity.logEvent` + engine's `publishEventSafe`; no second pipe | no duplication |
| Unused exports: `catalog/schema.js` {MANIFEST_DIR, VAR_KEY_RE, REPO_RE, VARIABLE_KINDS, BUILTIN_VARS}, `containers/policy.js` {BLOCKED_HOST_PATHS, SENSITIVE_HOST_PATHS, BLOCKED_CAPABILITIES, DANGEROUS_CAPABILITIES}, `containers/spec.js` {SPEC_DEFAULTS, parseVolumeString, parseDeviceString, sizeToBytes} | DEAD_CODE (exported constants/helpers kept public for tests/inspection; harmless) — SAFE_TO_CONSOLIDATE later |
| `containers/recreate.js#incompleteTransactions` (boot-recovery report, no caller yet) | NEEDS_FOLLOW_UP (wire into `/api/health` or System page) |
| `_reset*` test hooks | KEEP_SEPARATE (test-only) |
| Placeholders/stubs | none found in 10D code |

Regression at review time: `npm test` 922/922 · `test:web` 81/81 · `verify` 68/68 (no engine) and 110/110 (mock engine) · typecheck ✓ · build ✓ · `git diff --check` ✓ · `source-scan` ✓ · `smoke:routes` 27/27 · 10D suites 16+19+15+15+13, 10C 19.
